    ...
    if (mainnetEvent === undefined && xdaiEvent) {
      ...
      const mainnetNext =
        (mainnetEvent[orderBy.type] < xdaiEvent[orderBy.type] &&
          orderBy.order === OrderDirection.ascending.val) ||
        (mainnetEvent[orderBy.type] > xdaiEvent[orderBy.type] &&
...
async function getEventsBySubgraphFirst(
  mainnetSkip,
  xdaiSkip,
  orderBy,
  missingAmount
  ...
  let [mainnetEvents, xdaiEvents] = await Promise.all([
    getMainnetEvents(batchSize, mainnetSkip, orderBy),
    getxDaiEvents(batchSize, xdaiSkip, orderBy),
  ]);
  let { subgraphEvents } = reduceSubgraphEvents(
    mainnetEvents,
    xdaiEvents,
    orderBy
  );
  ...
  for (let i = 0; i < eventIds.length; i += 1000) {
    const eventIdsSlice = eventIds.slice(
      i,
  ...
  let { limitedEvents, _mainnetIndex, _xdaiIndex } = limitSubgraphEvents(
    subgraphEvents,
    missingAmount
  );
...
async function getEventsByApiFirst(
  apiSkip,
  orderBy,
  privateEvents,
  nameFilter,
  ...
  let [mainnetEvents, xdaiEvents] = await Promise.all([
    getMainnetEventsByIds(event_ids),
    getxDaiEventsByIds(event_ids),
  ]);
  const subgraphEvents = _.concat(
    mainnetEvents.data.events,
    xdaiEvents.data.events
  );
  normalizeSubgraphEvents(subgraphEvents);
  aggregateEventsData(apiEvents, subgraphEvents);
  const { limitedEvents, _apiIndex, _invalidEventsAmount } = limitApiEvents(
    apiEvents,
    Math.min(missingAmount, paginatedResults.total)
  );
  return {
    _events: limitedEvents,
    apiIndex: _apiIndex,
...
export async function getIndexPageData(
  ...
  let events = [],
    loopLimit = 10,
    total = 0,
    invalidEventsAmount = 0;
  while (events.length < PAGE_LIMIT && loopLimit > 0) {
    const missingAmount = PAGE_LIMIT - events.length;
    if (
      orderBy.type === OrderType.id.val ||
      orderBy.type === OrderType.date.val ||
      orderBy.type === OrderType.city.val
    ) {
      const {
        _events,
        apiIndex,
      ...
      let { _events, mainnetIndex, xdaiIndex } = await getEventsBySubgraphFirst(
        mainnetSkip,
        xdaiSkip,
        orderBy,
        missingAmount
